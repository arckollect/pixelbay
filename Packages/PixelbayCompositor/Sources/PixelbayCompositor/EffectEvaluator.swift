...
public enum EffectEvaluator {
    ...
    /// `trajectory`, the centre follows it via Catmull-Rom interpolation
    /// across `(t, x, y)` waypoints (keyframe-local time, `t=0` at
    /// `timelineRange.start`) so velocity stays continuous across sample
    /// boundaries — linear interpolation produces kinks at each waypoint
    /// that read as jitter at zoom factors ≥ 1.5×. With only ≤ 2 samples
    /// (no outer neighbours available) falls back to linear; at the
    /// trajectory's first / last segment the missing outer neighbour is
    /// mirrored from the boundary sample. The lookup `t` is clamped to
    /// `[first.t, last.t]` so we never extrapolate. Falls back to the
    /// static `(centerX, centerY)` for manually-authored keyframes
    /// (`trajectory == nil`) and for the empty-explicit sentinel the
    /// slice-2 auto-zoom path can emit (`trajectory == []`).
    private static func zoomCenter(
        ...
        // at 30 Hz → tens of samples). Switch to binary search if profiles
        // ever say otherwise.
        for i in 1..<trajectory.count {
            ...
            if localT <= b.t {
                ...
                let x = catmullRom(p0.x, a.x, b.x, p3.x, u: u)
                let y = catmullRom(p0.y, a.y, b.y, p3.y, u: u)
                return (x, y)
            }
        }
        return (lastSample.x, lastSample.y)
    ...
    private static func catmullRom(
        _ p0: Double, _ p1: Double, _ p2: Double, _ p3: Double, u: Double
    ) -> Double {
        let u2 = u * u
        let u3 = u2 * u
        return 0.5 * (
