...
final class AutoZoomServiceTests: XCTestCase {
    ...
    // MARK: - filterClicks dwell filter (slice #11.d)

    func test_filterClicks_dropsClicksDuringFastCursorMovement() {
        // Build a trajectory where the cursor is racing across the screen
        // from t=0..1s (covers 100% of the screen in 1s → 1.0 norm/s, well
        // above the 0.10 default threshold).
        let moving = (0...10).map {
            MouseTrajectorySample(
                timelineTime: 0.1 * Double($0),
                centerX: 0.1 * Double($0),
                centerY: 0.0
            )
        }
        // A click at t=0.95 — cursor was moving fast in the prior 0.25s.
        let clicks = [AutoZoomClick(timelineTime: 0.95)]
        let filtered = AutoZoomService.filterClicks(clicks, masterTrajectory: moving)
        XCTAssertEqual(filtered.count, 0, "click during fast cursor movement should be dropped")
    }

    func test_filterClicks_keepsClicksDuringDwell() {
        // Cursor sits still at (0.5, 0.5) from t=0..2s.
        let still = (0...20).map { _ in
            MouseTrajectorySample(timelineTime: 0.0, centerX: 0.5, centerY: 0.5)
        }
        // Re-timestamp so trajectory is monotonic across the dwell.
        let stillFixed = still.enumerated().map { (i, s) in
            MouseTrajectorySample(timelineTime: 0.1 * Double(i), centerX: s.centerX, centerY: s.centerY)
        }
        let clicks = [AutoZoomClick(timelineTime: 1.5, centerX: 0.5, centerY: 0.5)]
        let filtered = AutoZoomService.filterClicks(clicks, masterTrajectory: stillFixed)
        XCTAssertEqual(filtered.count, 1, "click during cursor dwell should be kept")
    }

    func test_filterClicks_nilTrajectory_returnsInputUnchanged() {
        let clicks = [
            AutoZoomClick(timelineTime: 1.0),
            AutoZoomClick(timelineTime: 2.0),
            AutoZoomClick(timelineTime: 3.0)
        ]
        let filtered = AutoZoomService.filterClicks(clicks, masterTrajectory: nil)
        XCTAssertEqual(filtered.count, 3, "graceful degradation when trajectory is unavailable")
    }

    // MARK: - decelerationZooms (slice #11.d)

    func test_decelerationZooms_emitsAtFastToSlowTransition() {
        // Fast moves t=0..0.5s, then stationary t=0.5..1.5s.
        var traj: [MouseTrajectorySample] = []
        for i in 0...5 {
            traj.append(MouseTrajectorySample(
                timelineTime: 0.1 * Double(i),
                centerX: 0.1 * Double(i),
                centerY: 0.0
            ))
        }
        for i in 6...20 {
        ...
        let decel = AutoZoomService.decelerationZooms(from: traj)
        XCTAssertEqual(decel.count, 1)
        XCTAssertGreaterThan(decel[0].timelineTime, 0.4)
        XCTAssertLessThan(decel[0].timelineTime, 1.0)
        XCTAssertEqual(decel[0].centerX, 0.5, accuracy: 0.05)
    }

    func test_decelerationZooms_emptyForNilOrShortTrajectory() {
        XCTAssertEqual(AutoZoomService.decelerationZooms(from: nil), [])
        XCTAssertEqual(AutoZoomService.decelerationZooms(from: [
            MouseTrajectorySample(timelineTime: 0, centerX: 0, centerY: 0)
        ]), [])
    }

    func test_decelerationZooms_coalescesNearbyTriggers() {
        // Two fast→slow transitions only 1s apart — coalesceWindow default
        // is 2s, so the second one is suppressed.
        var traj: [MouseTrajectorySample] = []
        // Fast block 1 → slow at t≈0.6s
        for i in 0...5 {
            traj.append(.init(timelineTime: 0.1 * Double(i), centerX: 0.1 * Double(i), centerY: 0))
        }
        for i in 6...9 {
            traj.append(.init(timelineTime: 0.1 * Double(i), centerX: 0.5, centerY: 0))
        }
        // Fast block 2 → slow at t≈1.5s
        for i in 10...14 {
            traj.append(.init(timelineTime: 0.1 * Double(i), centerX: 0.5 + 0.1 * Double(i - 10), centerY: 0))
        }
        for i in 15...25 {
            traj.append(.init(timelineTime: 0.1 * Double(i), centerX: 0.9, centerY: 0))
        }
        let decel = AutoZoomService.decelerationZooms(from: traj, coalesceWindow: 2.0)
        XCTAssertEqual(decel.count, 1, "second transition within 2s coalesce window should be suppressed")
    }
}
